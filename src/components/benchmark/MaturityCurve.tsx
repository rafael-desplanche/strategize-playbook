...
export function MaturityCurve({ score, marketAverageScore = 2.8, label, note }: MaturityCurveProps) {
  ...
  const safeMarketScore = Number.isFinite(marketAverageScore) ? marketAverageScore : 2.8;
  const markerT = scoreToT(safeScore);
  const marketT = scoreToT(safeMarketScore);
  ...
  if (!stages.length) {
    return (
      <div className="rounded-2xl border border-border/60 bg-card/60 px-4 py-6 text-sm text-muted-foreground">
